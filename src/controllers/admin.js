  ...
  try {
    ...
    if (!user) {
      return res.status(404).json({
        message:
          "Sorry, the user you’re looking for doesn’t exist or is no longer available.",
      });
    }
    await User.deleteOne({ _id: id });
    if (user.role === "vendor") {
      await Shop.deleteOne({ _id: user.shop }); // delete the vendor's shop
      await Product.deleteMany({ shop: user.shop }); // delete all products belonging to that shop
    }
    return res.status(200).json({
      success: true,
      message: "Deletion complete — the user has been successfully removed", // Corrected message typo
    });
  } catch (error) {
    return res.status(400).json({ success: false, message: error.message });
  }
